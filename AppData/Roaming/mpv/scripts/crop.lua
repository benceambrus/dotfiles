local opts = {
    ...
    up_coarse="UP",
    down_coarse="DOWN",
    left_coarse_fc="CTRL+LEFT",
    right_coarse_fc="CTRL+RIGHT",
    up_coarse_fc="CTRL+UP",
    down_coarse_fc="CTRL+DOWN",
    fine_movement=1,
    left_fine="ALT+LEFT",
    right_fine="ALT+RIGHT",
    up_fine="ALT+UP",
    down_fine="ALT+DOWN",
    left_fine_fc="CTRL+ALT+LEFT",
    right_fine_fc="CTRL+ALT+RIGHT",
    up_fine_fc="CTRL+ALT+UP",
    down_fine_fc="CTRL+ALT+DOWN",
...
function draw_crop_zone()
    if needs_drawing then
        ...
        if opts.draw_shade and crop_first_corner then
            ...
            crop_cursor.y = first_corner.y + math.abs((crop_cursor.x - first_corner.x))/ratio
            if crop_cursor.y > video_dim.bottom_right.y then
                crop_cursor.y = video_dim.bottom_right.y
                crop_cursor.x = first_corner.x + math.abs(crop_cursor.y - first_corner.y)*ratio
            end
        ...
        if opts.draw_text then
            ...
            if crop_first_corner then
                ...
                text = string.format("%s (%dx%d) - %.3f", text,
                    math.abs(cursor_video.x - crop_first_corner.x),
                    math.abs(cursor_video.y - crop_first_corner.y),
                    (cursor_video.x - crop_first_corner.x)/(cursor_video.y - crop_first_corner.y)
                )
                -- local ratio = math.abs((cursor_video.x - crop_first_corner.x)/(cursor_video.y - crop_first_corner.y))
                -- if ratio > 1.78 then
                --     text = text .. " - | q"
                -- elseif (ratio >= 1.77 and ratio <= 1.78) then
                --     text = text .. " - q"
                -- elseif (ratio > 0.89 and ratio < 1.77) then
                --     text = text .. " - q | h"
                -- elseif (ratio >= 0.88 and ratio <= 0.89) then
                --     text = text .. " - h"
                -- else
                --     text = text .. " - h |"
                -- end
...
function crop_video(x, y, w, h)
    ...
    table.insert(vf_table, crop)
    table.insert(saved_crops, crop)
...
function movement_func_selection(move_x, move_y) -- First corner move
    return function()
        crop_first_corner.x = crop_first_corner.x + move_x
        crop_first_corner.y = crop_first_corner.y + move_y
        crop_cursor.x = crop_cursor.x + move_x
        -- crop_cursor.y = crop_cursor.y - move_x
        needs_drawing = true
    end
end
...
bindings_repeat[opts.left_coarse_fc]  = movement_func_selection(-opts.coarse_movement, 0)
bindings_repeat[opts.right_coarse_fc] = movement_func_selection(opts.coarse_movement, 0)
bindings_repeat[opts.up_coarse_fc]    = movement_func_selection(0, -opts.coarse_movement)
bindings_repeat[opts.down_coarse_fc]  = movement_func_selection(0, opts.coarse_movement)
bindings_repeat[opts.left_fine_fc]    = movement_func_selection(-opts.fine_movement, 0)
bindings_repeat[opts.right_fine_fc]   = movement_func_selection(opts.fine_movement, 0)
bindings_repeat[opts.up_fine_fc]      = movement_func_selection(0, -opts.fine_movement)
bindings_repeat[opts.down_fine_fc]    = movement_func_selection(0, opts.fine_movement)
...
crop_id = 1
function change_crop_id()
    crop_id = crop_id + 1
    if crop_id > #saved_crops then
        crop_id = 1
    end
end
...
function toggle_crop()
    ...
    local vf_table = {}
    mp.set_property_native("vf", vf_table)
    table.insert(vf_table, 1, saved_crops[crop_id])
    mp.set_property_native("vf", vf_table)
    change_crop_id()
end
...
ratio = 0
function crop_half_screen()
    ratio = 0.888
    start_crop()
end

function crop_quarter_screen()
    ratio = 1.777
    start_crop()
end

-- mp.add_key_binding("c", "start-crop", start_crop)
mp.add_key_binding("c", "crop_half_screen", crop_half_screen)
mp.add_key_binding("x", "crop_quarter_screen", crop_quarter_screen)
mp.add_key_binding("s", "toggle-crop", toggle_crop)
mp.add_key_binding("d", "no_crop", no_crop)
mp.add_key_binding("a", "delete_crops", delete_crops)
